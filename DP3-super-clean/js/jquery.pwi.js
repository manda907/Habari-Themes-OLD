...
(function ($) {
        ...
        function albums(j) {
            ...
            while(i < settings.albumMaxResults && i< j.feed.entry.length){
                ...
                if ($.inArray($id_base, settings.albums) > -1 || settings.albums.length === 0) {
                    ...
                    settings.showAlbumTitles ? $scAlbum.append("<br/>" + j.feed.entry[i].title.$t + "<br/>" + (settings.showAlbumdate ? $album_date : "") + (settings.showAlbumPhotoCount ? "<br/>" + j.feed.entry[i].gphoto$numphotos.$t + " " + settings.labels.photos : "")) : false;
    ...
    $.fn.pwi.defaults = {
        ...
        photoSize: 800,
        maxResults: 60,
        showPager: 'both', //'top', 'bottom', 'both'
        thumbSize: 72,
        thumbCrop: 0,
        thumbCss: {
            'margin': '5px'
